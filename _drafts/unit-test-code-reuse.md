---
title: Unit Test Code Reuse with Vitest 4.1
tags: frontend
---
...
This is an updated version of *Unit Test Code Reuse with Vitest*, originally published June 30, 2025. Use the [previous version]({% link _posts/2025-06-30-unit-test-code-reuse.md %}) for Vitest 1.6 - 4.0. 

This version requires Vitest 4.1 or later.
{% endcapture %}
...
If your test needs to run inside a context, you can use an `aroundEach` hook that wraps around each test in the test suite.
...
The hook is passed a `runTest` function that *must* be called to run the test. If you declare multiple `aroundEach` hooks they nest inside each other.

If you just want code to run before and after tests, prefer using `beforeEach` with a cleanup return function.
