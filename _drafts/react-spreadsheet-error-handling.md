...
* Last time integrated `neverthrow` to add `Result<T,E>` types for Rust style error handling
* Used the `SpreadsheetData` method `setCellValueAndFormat` as a test case
* Now going to use for real in my `react-spreadsheet` component

# Spreadsheet Data Error

* First step is to replace the placeholder `string` error return with a proper error type.
...
* This isn't complete. Aim here is to put a framework in place. Family of error types defined as a discriminated union with a couple of starter types. Each one has a `type` field for the discriminated union and a `message` field that can be shown to the end user.
* There's `ValidationError` for cases where the data source restricts values that can be stored to specific types or ranges.
* For now, `StorageError` covers all the other cases where there are problems storing and persisting the values. It includes an optional http style status code that can be used to further classify the error. My main production implementation will have an http backend. Errors for other implementations can often be summarized with an http code too. Saves me from having to think too deeply about different error classes and data to return.

# Set and Is Valid

* I changed `setCellValueAndFormat` to use `Result<void,SpreadsheetDataError>`. I also added `isValidCellValueAndFormat`. This method let's you check whether a value is valid to use without trying to modify the data source. You'll see why it's needed later.
...
* Here's what the implementation looks like for my simple empty data source.

# Layered Data

* First use for `isValidCellValueAndFormat` is my layered data source. This allows you to layer an editable data source on top of a base reference data source. Whatever overriding value is stored in the edit layer needs to be compatible with the base layer.
...
* I'm not yet convinced that it's simpler and more readable than the imperative alternative.

# Boring Data

* I've [previously introduced you]({% link _posts/2024-10-07-react-spreadsheet-data-model.md %}) to the world's most boring spreadsheet data set.
* I've made it more interesting by adding some validation
...
* Now on to my `VirtualSpreadsheet` component
* I want the component to feel like a modern editor. Errors are highlighted but you're able to keep working normally. No modal pop ups. 

{% include candid-image.html src="/assets/images/react-spreadsheet/data-error-state.png" alt="Spreadsheet Data Error State" %}

* First step is to add React state to represent an error state

```ts
  const [dataError, setDataError] = React.useState<SpreadsheetDataError|null>(null);
```

* I added new CSS classes that are used when the component is in an error state
...
* Events which apply changes, like hitting `Enter` on the keyboard, now check whether the commit succeeded
...
* I started by using absolute positioning and trying to calculate the best place to put it relative to the focus cell. There were all kinds of problems when the cell scrolled off the screen or was close to the edge. Doing it properly would involve knowing the size of the tag which depends on the length of the text which would mean using an effect.
* I decided to let the browser do the layout by making the grid container a flex box. No impact on the other elements that make up the grid as they all use absolute positioning.
...
* The `alignItems` property is used to position the tag at the `start` or `end` of the grid depending on the position of the focus cell. 
...
* I also added test cases for a few edge cases I previously missed
* Now have coverage of all non-layout dependent functionality
* Added "Data Error"  story to Story book
...
* Had to exclude the story from Auto Docs as it doesn't play nicely with other stories. To be in the error state the the focus cell must be in edit mode with some incorrect text entered. When in edit mode it has the focus which means the browser scrolls the *Docs* page down past the main documentation so that it's in view. I use the keyboard event to modify the cell value. On the Docs page multiple stories are being rendered in parallel which means it's possible for the focus to change to another story between selecting the cell and sending the keyboard event.
